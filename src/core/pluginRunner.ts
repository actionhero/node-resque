export async function RunPlugins(self, type, func, queue, job, args, pluginCounter?) {
...
export async function RunPlugin(self, PluginRefrence, type, func, queue, job, args) {
  if (!job) { return true }
  ...
  if (typeof PluginRefrence === 'string') {
    const PluginConstructor = require(`./plugins/${PluginRefrence}.js`)[PluginRefrence]
