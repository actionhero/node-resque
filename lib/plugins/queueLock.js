// If a job with the same name, queue, and args is already running, do not enqueue it
...
queueLock.prototype.key = function(){
  ...
  if (self.options.key != null){
    ...
    return self.worker.connection.key('lock', self.func, self.queue, self.args.join("-"));
  }
}
