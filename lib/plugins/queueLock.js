...
queueLock.prototype.before_enqueue = function(callback){
  ...
  var self = this;
  var key = self.key();
  var lockTimeout = self.lock_timeout() * 1000; //in ms
  //setnx && expire
  self.queueObject.connection.redis.set(key, self.uniqueId, 'NX', 'PX', lockTimeout, function(err, setCallback){
...
queueLock.prototype.after_perform = function(callback){
  ...
  redis.get(key, function(err, uniqueId){
    ...
    if(self.uniqueId.toString() !== uniqueId){
      return callback(null, true);  //don't release lock
    }else{
      redis.watch(key).then(function(){
        redis.multi().del(key).exec(function(err/*, result*/){
          if(err){ return callback(err, true); }
          // if(result === null){ console.log("** release lock fail"); }
          callback(null, true);
        })
      });
    }
