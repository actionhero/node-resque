...
var queueLock = function(worker, job, args, options){
  var self = this;
  self.worker = worker;
  self.job = job;
  self.args = args;
...
queueLock.prototype.key = function(){
  ...
  if (self.options.key != null){
    ...
    return self.worker.connection.key('lock', self.worker.queue, self.job.class, self.args.sort().join("-"));
  }
}
...
queueLock.prototype.run = function(callback){
  // TODO: This should be around enqueue, not run!
  var self = this;
  var key = self.key();
...
queueLock.prototype.jobComplete = function(callback){
