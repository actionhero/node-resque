...
var scheduler = function(options, callback){
  ...
  self.connection.connect(function(){
    self.queue = new queue({connection: options.connection, queue: ""}, function(){
...
scheduler.prototype.enqueueDelayedItemsForTimestamp = function(timestamp, callback) {
  ...
  self.nextItemForTimestamp(timestamp, function(err, job){
    if (!err && job && self.running === true) {
      self.transfer(timestamp, job);
...
scheduler.prototype.transfer = function(timestamp, job) {
  var self = this;
  self.emit("enquing_job", timestamp, job);
  self.queue.options.queue = job.queue;
  self.queue.enqueue(job["class"], job.args);
};
