...
var runPlugin = function(pluginName, type, func, job, args, callback){
  var self = this;
  process.nextTick(function(){
    if(job == null){
      callback(null, true);
    }else{
      if(self.jobs[func]['pluginOptions'] != null){
      ...
      var plugin = new pluginConstructor(self, func, job, args, pluginOptions);
...
var runPlugins = function(type, func, job, args, callback, pluginCounter){
  var self = this;
  ...
  if(job == null){
    ...
    self.runPlugin(pluginName, type, func, job, args, function(err, toRun){
      pluginCounter++;
      if(toRun === false){
        callback(err, false);
      }else{
        self.runPlugins(type, func, job, args, callback, pluginCounter);
      }
    });
  }
}
