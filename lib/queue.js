...
queue.prototype.enqueue = function(func, args, callback){
  var self = this;
  var queue = self.options.queue;
  self.connection.redis.sadd(self.connection.key('queues'), queue, function(){
...
queue.prototype.enqueueAt = function(timestamp, func, args, callback){
  var self = this;
...
queue.prototype.enqueueIn = function(time, func, args, callback){
  var self = this;
  var timestamp = (new Date().getTime()) + time;
  self.enqueueAt(timestamp, func, args, function(){
    if(typeof callback == "function"){ callback(); }
  });
}
