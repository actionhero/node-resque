...
class Queue extends EventEmitter {
  ...
  async enqueue (q, func, args) {
    ...
    let toRun = await PluginRunner.RunPlugins(this, 'before_enqueue', func, q, job, args)
    ...
    await PluginRunner.RunPlugins(this, 'after_enqueue', func, q, job, args)
    return toRun
  }
  ...
  async allWorkingOn () {
    ...
    for (let w in Object.keys(workers)) {
      results[w] = 'started'
  ...
  async cleanOldWorkers (age) {
    ...
    for (let workerName in Object.keys(data)) {
