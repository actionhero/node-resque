...
var pluginRunner = require(__dirname + "/pluginRunner.js").pluginRunner;
...
var worker = function(options, jobs, callback){
  ...
  self.runPlugin  = pluginRunner.prototype.runPlugin
  self.runPlugins = pluginRunner.prototype.runPlugins
}

util.inherits(worker, EventEmitter);
// util.inherits(worker, pluginRunner);
...
worker.prototype.perform = function(job, callback) {
  ...
  if (cb != null) {
    ...
    d.run(function(){
      self.working = true;
      self.job = job;
      self.runPlugins('before_perform', job["class"], self.jobs[job["class"]], job.args, function(err, toRun){
        if(toRun == false){
          ...
          cb.apply(null, [].slice.call(args).concat([function(result) {
            self.runPlugins('after_perform', job["class"], self.jobs[job["class"]], job.args, function(err, toRun){
...
// worker.prototype.runWith = function(plugins, callback){
//   var self = this;
...
//       var pluginConstructor = require(__dirname + "/plugins/worker/" + pluginName + ".js")[pluginName];
//       var plugin = new pluginConstructor(self, job, function(error, toRun, jobCallback){
//         if(error != null){
//           self.completeJob(null, err, callback);
//         }else if(toRun === false){
//           self.reEnqueue(job, pluginName, function(){
//             self.completeJob(null, null, callback);
//           });
//         }else{
//           self.activePlugins.push(plugin);
//           self.runWith(plugins, callback);
//         }
//       });
//       plugin.run();
//     });
...
worker.prototype.reEnqueue = function(job, plugin, callback){
  var self = this;
  self.emit("reEnqueue", self.queue, job, plugin);
  self.queueObject.enqueue(self.queue, job.class, self.job.args, function(){
    setTimeout(function(){
      callback();
...
worker.prototype.checkQueues = function(callback){
  ...
  if (self.ready === true && ( self.queues === '*' || self.queues.length === 0 )) {
