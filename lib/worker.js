...
class Worker extends EventEmitter {
  constructor (options, jobs) {
    ...
    this.queueObject.on('error', (error) => { this.emit('error', null, null, error) })
  }
  ...
  async poll (nQueue) {
    ...
    if (this.working === true) {
      ...
      this.emit('error', this.queue, null, error)
      return null
    }
  ...
  async perform (job) {
    ...
    try {
      toRun = await PluginRunner.RunPlugins(this, 'before_perform', job['class'], this.queue, this.jobs[job['class']], job.args)
      ...
      toRun = await PluginRunner.RunPlugins(this, 'after_perform', job['class'], this.queue, this.jobs[job['class']], job.args)
  ...
  async performInline (func, args) {
    ...
    try {
      toRun = await PluginRunner.runPlugins(this, 'before_perform', func, q, this.jobs[func], args)
      if (toRun === false) { return }

      this.result = await this.jobs[func].perform.apply(this, args)
      toRun = await PluginRunner.runPlugins(this, 'after_perform', func, q, this.jobs[func], args)
      return this.result
    } catch (error) {
      this.error = error
      throw error
  ...
  async checkQueues () {
    ...
    if (this.ready === true && this.queues.length > 0 && this.queues.shift) {
      return
    }
