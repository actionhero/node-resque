...
worker.prototype.untrack = function(name, queues, callback) {
  var self = this;
  self.connection.redis.srem(self.connection.key('workers'), (self.name + ":" + queues), function(){
...
worker.prototype.checkQueues = function(callback){
  ...
  if (( self.queues[0] === '*' && self.queues.length == 1 ) || self.queues.length === 0 || self.originalQueue == "*" ) {
    self.originalQueue = "*";
    self.untrack(self.name, self.stringQueues(), function(){
