...
// If you are planning on running a job via #performInline, this worker should also not be started, nor should you expec to be using event emitters to monitor this worker
...
worker.prototype.init = function(callback) {
  ...
  var args, _ref;
  self.track();
  self.connection.redis.set(self.connection.key('worker', self.name, self.stringQueues(), 'started'), Math.round((new Date()).getTime() / 1000), function(){
    if(typeof callback === 'function'){ callback(); }
  });
};
